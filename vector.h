...
private:
    template <typename... Args>
    iterator RellocateAndPutElm(size_t shift, Args&&... args);
...
T& Vector<T>::EmplaceBack(Args&&... args) {
    ...
    if (size_ == Capacity()) {
        result = RellocateAndPutElm(size_, std::forward<Args>(args)...);
    } 
    else result = new (data_ + size_) T(std::forward<Args>(args)...);
...
typename Vector<T>::iterator Vector<T>::Emplace(const_iterator pos, Args&&... args) {
    ...
    if (size_ == Capacity()) {
        result = RellocateAndPutElm(shift, std::forward<Args>(args)...);
    } else {
        if (size_ != 0) {
        ...
        result = new (data_.GetAddress() + shift) T(std::forward<Args>(args)...);
    }
...
template <typename T>
template <typename... Args>
typename Vector<T>::iterator Vector<T>::RellocateAndPutElm(size_t shift, Args&&... args) {
    iterator result = nullptr;
    
    RawMemory<T> new_data(size_ == 0 ? 1 : size_ * 2);
    result = new (new_data.GetAddress() + shift) T(std::forward<Args>(args)...);
    // constexpr оператор if будет вычислен во время компиляции
    if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>) {
